...
$portalMaxId = (int)Yii::$app->db->createCommand('SELECT COALESCE(MAX(request_id), 0) FROM smisportal.fss_refund_requests')->queryScalar();
$smisMaxId = (int)Yii::$app->smisDb->createCommand('SELECT COALESCE(MAX(request_id), 0) FROM smis.fss_refund_requests')->queryScalar();
$requestId = max($portalMaxId, $smisMaxId) + 1;
...
$smisRow = $row;
unset($smisRow['payment_method'], $smisRow['sync_status'], $smisRow['sync_error'], $smisRow['last_synced_at']);
...
try {
    ...
    Yii::$app->smisDb->createCommand()
        ->insert('smis.fss_refund_requests', $smisRow)
        ->execute();
    ...
    echo "SMIS fss_refund_requests record created. Request ID: $requestId\n";
    echo "SUCCESS: Application submitted to both Portal and SMIS.\n";
} catch (\Throwable $e) {

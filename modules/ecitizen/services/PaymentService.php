...
class PaymentService
{
    ...
    /**
     * Reads the eCitizen-relevant payment types straight out of
     * smisportal.fss_payment_types - matched by description against the NDU
     * service codes workbook. Rows are never seeded/written here; the table is
     * expected to already hold a row for every service in the workbook.
     */
    public function paymentTypes(): array
    {
        $catalog = $this->serviceCatalog();
        $descriptions = array_map(static fn (string $d): string => substr($d, 0, 150), $catalog);

        $paymentTypes = (new Query())
            ->select(['payment_type_id', 'payment_desc'])
            ->from('smisportal.fss_payment_types')
            ->where(['payment_desc' => array_values($descriptions)])
            ->all($this->db);
        $order = array_flip(array_map(self::normalizeDescription(...), $descriptions));
        usort(
            $paymentTypes,
            static fn (array $a, array $b): int =>
                ($order[self::normalizeDescription((string) $a['payment_desc'])] ?? PHP_INT_MAX)
                <=> ($order[self::normalizeDescription((string) $b['payment_desc'])] ?? PHP_INT_MAX)
        );
    ...
    public function serviceIdForPaymentType(int $paymentTypeId): string
    {
        ...
        $normalizedTarget = self::normalizeDescription((string) $description);
        foreach ($this->serviceCatalog() as $serviceCode => $catalogDescription) {
            if (self::normalizeDescription(substr($catalogDescription, 0, 150)) === $normalizedTarget) {
    ...
    private function ensureFeeTransactionDescription(array $slip, float $amount, string $paymentDate, string $paymentDescription): void
    {
        ...
        $this->advanceFeeTransactionSequence((int) $slip['trans_id']);
    }

    /**
     * ensureFeeTransactionDescription() inserts fss_fee_transactions rows with
     * an explicit trans_id borrowed from the banking slip's own primary key,
     * bypassing this table's own trans_id auto-increment sequence. Left
     * unchecked, that sequence falls behind the highest trans_id actually in
     * use and eventually hands out a value already taken by one of those
     * explicit inserts (a unique constraint violation on fss_fee_transactions_
     * pkey — see creditPortalFeeStatement(), which relies on the sequence for
     * its own, non-explicit inserts). Bumping it here after every explicit
     * insert keeps it ahead permanently.
     */
    private function advanceFeeTransactionSequence(int $transId): void
    {
        ...
        $current = $this->db->createCommand("SELECT last_value, is_called FROM {$seq}")->queryOne();
        $nextWouldBe = ((bool) $current['is_called']) ? ((int) $current['last_value'] + 1) : (int) $current['last_value'];
        if ($nextWouldBe > $transId) {
            return;
        }

        $this->db->createCommand('SELECT setval(:seq, :val, true)', [':seq' => $seq, ':val' => $transId])->execute();
    }

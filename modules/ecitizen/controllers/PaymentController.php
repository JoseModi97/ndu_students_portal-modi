...
final class PaymentController extends BaseController
{
    ...
    public function actionCheckout(): string|Response|array
    {
        ...
        try {
            ...
            $description = $paymentType['payment_desc'] ?? 'eCitizen student fee payment';
    ...
    private function normalizeInvoices(array $invoices): array
    {
        $normalized = array_map(static function (array $invoice): array {
            ...
            $isSettled = in_array($postStatus, ['NOT POSTED', 'POSTED'], true) || !empty($invoice['has_fee_payment']);
            $invoice['settlement_status'] = $isSettled ? 'Settled' : 'Not settled';
            $invoice['action_status'] = match (true) {
                !empty($invoice['has_fee_payment']) || $postStatus === 'POSTED' => 'Posted',
                $postStatus === 'NOT POSTED' => 'Awaiting finance posting',
        ...
        usort($normalized, static function (array $left, array $right): int {
            $leftDate = strtotime((string) ($left['deposit_date'] ?? '')) ?: 0;
            $rightDate = strtotime((string) ($right['deposit_date'] ?? '')) ?: 0;
    ...
    private function invoiceFilterOptions(array $invoices): array
    {
        ...
        foreach ($invoices as $invoice) {
            ...
            foreach (compact('reference', 'date', 'amount', 'settlementStatus', 'comment', 'actionStatus') as $key => $value) {
                if ($value === '') {
                    continue;
                }
                $attribute = match ($key) {
    ...
    public function actionInvoice(int $trans_id): string|Response
    {
        ...
        if (!empty($invoice['has_fee_payment']) || strtoupper((string) $invoice['post_status']) === 'POSTED') {
            $this->setFlash('danger', 'Already posted', 'This invoice has already been posted and cannot be relaunched.');
            return $this->redirect(['invoices']);
        }

        if (strtoupper((string) $invoice['post_status']) === 'NOT POSTED') {
            $this->setFlash('info', 'Payment confirmed', 'This payment is already confirmed and awaiting finance posting.');
            return $this->redirect(['invoices']);
        }
    ...
    public function actionCompletePayment(int $trans_id): Response
    {
        ...
        $this->setFlash('success', 'Payment confirmed', 'eCitizen confirmed the payment. It is now awaiting finance posting.');
        return $this->redirect(['invoices']);
    }
